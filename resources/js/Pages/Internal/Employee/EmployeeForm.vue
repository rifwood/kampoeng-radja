...
const supervisorOptions = computed(() => {
    ...
    return props.masterData.karyawan.filter((item) =>
        `${item.nama} ${item.nik}`.toLocaleLowerCase("id").includes(term),
    );
});
const selectSignature = (event) => {
    ...
    signaturePreviewUrl.value =
        signatureObjectUrl ?? props.employee?.signaturePhotoUrl ?? null;
};
onBeforeUnmount(() => {
    if (signatureObjectUrl) URL.revokeObjectURL(signatureObjectUrl);
});
...
const submit = async () => {
    ...
    if (editing.value) {
        form.post(
            route("dashboard.karyawan.update", props.employee.id),
            options,
        );
    } else {
    ...
    <form class="space-y-5" @submit.prevent="submit">
        ...
        >
            <div class="mb-5 border-b border-slate-100 pb-4">
                ...
                <p class="mt-1 text-xs text-slate-500">
                    Identitas utama karyawan sesuai dokumen resmi.
                </p>
            </div>
            <div class="grid gap-4 md:grid-cols-2">
                ...
                <label :class="fields.label"
                    >Jenis Kelamin
                    <select v-model="form.jenis_kelamin" :class="fields.input">
                        <option value="L">Laki-laki</option>
                        <option value="P">Perempuan</option></select
                    ><span
                        v-if="form.errors.jenis_kelamin"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.jenis_kelamin }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Tempat Lahir
                ...
                <label :class="fields.label"
                    >Agama
                    <select v-model="form.agama" :class="fields.input">
                        <option
                            v-for="item in [
                                'islam',
                                'kristen',
                ...
                <label :class="fields.label"
                    ...
                    ><span
                        v-if="form.errors.status_perkawinan"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.status_perkawinan }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Pendidikan
                    <select v-model="form.pendidikan" :class="fields.input">
                        <option
                            v-for="item in [
                                'SD',
                                'SMP',
                    ...
                    ><span
                        v-if="form.errors.pendidikan"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.pendidikan }}</span
                    ></label
                >
                <label :class="fields.label"
                    >No. HP
                    ...
                    /><span
                        ...
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.no_hp }}</span
                    ></label
                >
                <label :class="[fields.label, 'md:col-span-2']"
                    >Alamat
                    <textarea
                        v-model="form.alamat"
                        :class="fields.input"
        ...
        >
            <div class="mb-5 border-b border-slate-100 pb-4">
                ...
                <p class="mt-1 text-xs text-slate-500">
                    Penempatan, status kerja, dan periode bekerja.
                </p>
            </div>
            <div class="grid gap-4 md:grid-cols-2">
                <label :class="fields.label"
                    >Jabatan <span class="text-red-500">*</span
                    ><select v-model="form.jabatan_id" :class="fields.input">
                    ...
                    ><span
                        v-if="form.errors.jabatan_id"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.jabatan_id }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Departemen
                    <select v-model="form.departemen_id" :class="fields.input">
                    ...
                    ><span
                        ...
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.departemen_id }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Penempatan
                    <select v-model="form.penempatan_id" :class="fields.input">
                        <option value="">Tanpa penempatan</option>
                        <option
                    ...
                    ><span
                        v-if="form.errors.penempatan_id"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.penempatan_id }}</span
                    ></label
                >
                <div>
                    <label :class="fields.label"
                    ...
                    /></label>
                    <select
                        v-model="form.atasan_langsung_id"
                        :class="fields.input"
                        class="mt-2"
                    >
                    ...
                    <span
                        v-if="form.errors.atasan_langsung_id"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.atasan_langsung_id }}</span
                    >
                </div>
                <label :class="fields.label"
                    >Status Kerja
                    <select v-model="form.status_kerja" :class="fields.input">
                        <option
                            v-for="item in [
                                'kontrak',
                                'magang',
                    ...
                    ><span
                        v-if="form.errors.status_kerja"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.status_kerja }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Status Keaktifan
                    <select
                        v-model="form.status_keaktifan"
                        :class="fields.input"
                    >
                        <option value="aktif">Aktif</option>
                        <option value="nonaktif">Nonaktif</option></select
                    ><span
                        v-if="form.errors.status_keaktifan"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.status_keaktifan }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Tanggal Masuk
                    ...
                    /><span
                        ...
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.tanggal_masuk }}</span
                    ></label
                >
                <label :class="fields.label"
                    >Tanggal Keluar
                    ...
                    /><span
                        ...
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.tanggal_keluar }}</span
                    ></label
                >
            </div>
        </section>
        ...
        >
            <div class="mb-5 border-b border-slate-100 pb-4">
                <h2 class="text-base font-bold text-[#15356f]">Dokumen</h2>
                <p class="mt-1 text-xs text-slate-500">
                    File disimpan privat dan hanya dapat dilihat Super Admin.
                </p>
            </div>
            <label :class="fields.label"
                >Foto KTP
                ...
                ><input
                    ...
                    @change="form.foto_ktp = $event.target.files[0]"
                /><span
                    v-if="editing && employee.hasKtpPhoto"
                    class="mt-2 block text-xs text-emerald-600"
                    >Dokumen lama tersedia. Pilih file hanya jika ingin
                    mengganti.</span
                ><span
                    v-if="form.errors.foto_ktp"
                    class="mt-1 block text-xs text-red-600"
                    >{{ form.errors.foto_ktp }}</span
                ></label
            >
            <div class="mt-5 border-t border-slate-100 pt-5">
                <label :class="fields.label"
                    >Foto Tanda Tangan
                    <span class="font-normal text-slate-400"
                        >(JPG atau PNG — maks. 5 MB)</span
                    ><input
                        :class="fields.input"
                        type="file"
                        accept="image/jpeg,image/png"
                        @change="selectSignature"
                    /><span
                        v-if="form.errors.foto_tanda_tangan"
                        class="mt-1 block text-xs text-red-600"
                        >{{ form.errors.foto_tanda_tangan }}</span
                    ></label
                >
                <div
                    v-if="signaturePreviewUrl"
                    class="mt-3 inline-flex min-h-28 min-w-52 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 p-3"
                >
                    <img
                        :src="signaturePreviewUrl"
                        alt="Preview tanda tangan"
                        class="max-h-28 max-w-64 object-contain"
                    />
                </div>
                <p
                    v-if="editing && employee.hasSignaturePhoto"
                    class="mt-2 text-xs text-emerald-600"
                >
                    Tanda tangan lama dipertahankan jika tidak memilih file
                    baru.
                </p>
            </div>
        </section>
        ...
        <div class="flex flex-wrap justify-end gap-3">
            <Link
                :href="
                    editing
                        ? route('dashboard.karyawan.show', employee.id)
