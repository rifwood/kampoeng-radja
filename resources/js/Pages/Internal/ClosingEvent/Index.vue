...
const exportUrl = computed(() =>
    route("dashboard.closing-event.export", {
        bulan: filter.bulan,
        tahun: filter.tahun,
    }),
);
...
const exportPeriodLabel = computed(
    () => `${months[Number(filter.bulan) - 1] ?? filter.bulan} ${filter.tahun}`,
);

const applyFilter = () =>
    router.get(route("dashboard.closing-event.index"), filter, {
        preserveState: true,
        replace: true,
    });
...
const remove = async (item) => {
    const confirmed = await confirm({
        ...
        message: `Apakah Anda yakin ingin menghapus data ${item.konsumen}?`,
        description: "Tindakan ini tidak dapat dibatalkan.",
        confirmText: "Ya, Hapus",
    });
    if (confirmed)
        router.delete(route("dashboard.closing-event.destroy", item.id));
};
</script>
...
<template>
    ...
    >
        <div class="w-full max-w-none px-4 py-6 sm:px-6 lg:px-7">
            ...
            <section
                class="mb-5 rounded-xl border border-slate-200 bg-white px-4 py-3.5 shadow-sm sm:px-5"
            >
                <div
            ...
            >
                <div class="overflow-x-auto 2xl:overflow-x-visible">
                    <table
                        class="w-full min-w-[1220px] table-fixed text-left text-[11px] 2xl:min-w-full"
                    >
                        <colgroup>
                            <col class="w-[14%]" />
                            <col class="w-[6%]" />
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            >
                                <td
                                    class="whitespace-nowrap px-3 py-3.5 font-semibold text-slate-700"
                                >
                                    <span class="block">{{
                                        item.tanggalLabel
                                    }}</span>
                                    ...
                                    >
                                        <span
                                            class="ongoing-status-dot h-1.5 w-1.5 shrink-0 rounded-full bg-orange-500"
                                            aria-hidden="true"
                                        />
                                        Sedang Berlangsung
                                    </span>
                                ...
                                <td
                                    class="truncate px-2 py-3.5 font-semibold text-slate-700"
                                    :title="item.pic"
                                >
                                    {{ item.pic }}
                                </td>
                                ...
                                <td class="px-2 py-3.5">
                                    <div class="flex min-w-0 flex-wrap gap-1">
                                        <span
                                            v-for="place in item.lokasi.slice(
                                                0,
                                                2,
                                            )"
                                            :key="place.id"
                                ...
                                <td
                                    class="whitespace-nowrap px-2 py-3.5 text-slate-600"
                                >
                                    {{ item.jamKedatangan }}
                                </td>
                                <td
                                    class="px-2 py-3.5 text-center font-semibold tabular-nums text-slate-700"
                                >
                                ...
                                <td
                                    class="px-2 py-3.5 align-middle text-slate-600"
                                >
                                    <span
                                        class="line-clamp-2 leading-4"
                                        :title="item.additional || '-'"
                                    >
                                        {{ item.additional || "-" }}
                                    </span>
                                </td>
                            ...
                            <tr v-if="events.data.length === 0">
                                <td colspan="11" class="px-6 py-14 text-center">
                                    ...
                                    <Link
                                        ...
                                        :href="
                                            route(
                                                'dashboard.closing-event.create',
                                            )
                                        "
                ...
                >
                    <span
                        >Menampilkan {{ events.from ?? 0 }}–{{
                            events.to ?? 0
                        }}
                        dari {{ events.total }} data</span
                    >
                    <div
