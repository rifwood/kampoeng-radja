...
const props = defineProps({ hero: Object, news: Array, promotions: Array, promotionFallbackEnabled: Boolean, products: Array, partners: Array, featuredRides: Array, featuredRideFallbackEnabled: Boolean });
...
const cancelHeroSchedule = () => {
  if (heroIdleHandle !== null && 'cancelIdleCallback' in window) window.cancelIdleCallback(heroIdleHandle);
...
const scheduleHeroVideo = () => {
  ...
  if ('requestIdleCallback' in window) {
    heroIdleHandle = window.requestIdleCallback(activateHeroVideo, { timeout: 1200 });
  } else {
...
const newsCards = computed(() => (props.news ?? []).map((item) => ({
...
<template>
  ...
  <PublicLayout>
    ...
    <section class="home-hero relative z-10 min-h-[620px] overflow-hidden bg-[#304755] sm:min-h-[680px] lg:h-[720px]">
      <img
        v-if="heroPosterUrl"
        :src="heroPosterUrl"
        alt=""
    ...
    <section ref="newsSection" class="bg-[#f8f9fa] px-5 pb-20 pt-32 lg:px-0"><div class="mx-auto max-w-[1120px]"><h2 class="font-heading text-[28px] font-bold text-[#062a59]">Media & Berita</h2><p class="mt-2 text-sm text-[#4b5563]">Informasi terbaru seputar Kampoeng Radja.</p><div v-if="newsCards.length" class="mt-5 grid items-start gap-5 md:grid-cols-3"><article v-for="(newsItem,index) in newsCards" :key="newsItem.title" class="overflow-hidden rounded-xl border border-[#e1e2eb] bg-white"><div class="relative"><LazyImage :src="newsItem.foto_url" :active="newsMediaReady" :alt="newsItem.title" class-name="h-[198px] w-full object-cover" /><span :class="newsItem.categoryClass" class="absolute left-3 top-3 rounded-full px-3 py-1 text-[10px] text-white">{{ newsItem.category }}</span></div><div class="p-[18px]"><h3 class="font-heading text-xl font-bold leading-6 text-[#062a59]">{{ newsItem.title }}</h3><p class="mt-3 text-sm leading-5 text-[#4b5563]">{{ newsItem.excerpt }}</p><p v-if="expandedNews === index" class="mt-4 border-t border-[#e1e2eb] pt-4 text-sm leading-6 text-[#4b5563]">{{ newsItem.detail }}</p><button type="button" class="mt-4 h-9 w-full rounded-md border border-[#0063c7] text-xs font-medium text-[#0063c7]" :aria-expanded="expandedNews === index" @click="expandedNews = expandedNews === index ? null : index">{{ expandedNews === index ? 'Tutup Detail' : 'Lihat Detail' }}</button></div></article></div><p v-else class="mt-8 rounded-xl border border-dashed border-[#d6dce6] bg-white px-6 py-10 text-center text-sm text-[#667085]">Belum ada Media &amp; Berita terbaru.</p></div></section>
    <section ref="promoSection" class="home-promo-section relative overflow-hidden px-4 py-16 text-white sm:px-6 sm:py-20 lg:px-8 lg:py-24">
        ...
        >
          ...
          <div v-else ref="promoViewport" class="overflow-hidden" :class="isPromoReady ? 'visible' : 'invisible'">
            ...
            >
              ...
              >
                <img :src="promoMediaReady ? promo.poster_url : undefined" :alt="`Poster ${promo.title}`" class="aspect-[4/5] w-full bg-slate-100 object-cover" loading="lazy" decoding="async" />
    ...
    <div v-if="selectedPromo" class="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/65 p-3 sm:p-6" role="dialog" aria-modal="true" :aria-label="`Detail ${selectedPromo.title}`" @click.self="closePromoDetail">
      <article class="relative grid max-h-[92vh] w-full max-w-[920px] overflow-y-auto rounded-[24px] bg-white shadow-2xl md:grid-cols-[minmax(280px,0.85fr)_minmax(0,1.15fr)]">
        ...
        <div class="grid min-h-[300px] place-items-center bg-slate-100 p-4 sm:p-6 md:min-h-[560px]">
          <img :src="selectedPromo.poster_url" :alt="`Poster ${selectedPromo.title}`" class="max-h-[520px] h-auto w-full object-contain" decoding="async" />
        </div>
    ...
    <section ref="ridesSection" class="bg-[#f1f3f5] px-5 py-20 lg:px-0"><div class="mx-auto max-w-[1120px]"><div class="flex flex-wrap items-end justify-between gap-5"><div><h2 class="font-heading text-[28px] font-bold text-[#191c1e]">Wahana Unggulan</h2><p class="mt-2 text-sm text-[#434655]">Jelajahi berbagai wahana seru yang siap menguji adrenalin dan memberikan keceriaan.</p></div><Link :href="route('wahana')" class="text-sm font-bold text-[#005cc8]">Lihat Semua Wahana →</Link></div><div class="mt-8 grid gap-6 md:grid-cols-3"><article v-for="ride in featuredCards" :key="ride.title" class="overflow-hidden rounded-[24px] border border-[#e0e3e5] bg-white shadow-[0_1px_2px_rgba(0,0,0,.05)]"><div class="relative"><LazyImage :src="ride.image" :active="ridesMediaReady" :alt="ride.title" class-name="h-[137px] w-full object-cover" /><span :class="ride.tone" class="absolute left-3 top-3 rounded-full px-3 py-1 text-[10px] font-bold">{{ ride.label }}</span></div><div class="p-[18px]"><h3 class="font-heading text-lg font-bold text-[#191c1e]">{{ ride.title }}</h3><p class="mt-2 min-h-10 text-xs leading-4 text-[#434655]">{{ ride.description }}</p><Link :href="route('wahana')" class="mt-4 block w-full rounded-md border border-[#0063c7] py-2 text-center text-xs font-medium text-[#0063c7]">Detail Wahana</Link></div></article></div></div></section>
    <section ref="partnersSection" class="overflow-hidden border-t-[3px] border-[#1685ed] bg-[#dedede] px-5 py-10 lg:px-0"><div class="mx-auto max-w-[1120px]"><h2 class="text-center font-heading text-xl font-bold text-[#191c1e]">Mitra</h2><div v-if="partners?.length" class="mt-8 overflow-hidden"><div class="animate-marquee flex w-max will-change-transform"><div class="flex shrink-0 items-center gap-16 pr-16"><div v-for="item in partners" :key="item.id" class="grid h-12 w-28 shrink-0 place-items-center"><LazyImage :src="item.logo" :active="partnersMediaReady" :alt="item.name" class-name="max-h-10 max-w-24 object-contain" /></div></div><div aria-hidden="true" class="flex shrink-0 items-center gap-16 pr-16"><div v-for="item in partners" :key="`duplicate-${item.id}`" class="grid h-12 w-28 shrink-0 place-items-center"><LazyImage :src="item.logo" :active="partnersMediaReady" alt="" class-name="max-h-10 max-w-24 object-contain" /></div></div></div></div><p v-else class="mt-6 text-center text-sm text-[#5e6470]">Belum ada Mitra yang ditampilkan.</p></div></section>
    <section ref="mapSection" class="bg-[#f9f9ff] px-5 py-[120px] lg:px-0"><div class="mx-auto h-[450px] max-w-[1120px] overflow-hidden rounded-2xl border-2 border-[#e1e2eb] bg-slate-100 shadow-[0_1px_2px_rgba(0,0,0,.05)]"><iframe v-if="mapReady" title="Lokasi Kampoeng Radja" class="h-[446px] w-full border-0" src="https://www.google.com/maps?q=kampoeng+radja&amp;z=14&amp;t=m&amp;hl=en&amp;output=embed" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe></div></section>
  </PublicLayout>
</template>
