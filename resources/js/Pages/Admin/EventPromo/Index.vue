...
const destroyItem = (item) => {
    if (!window.confirm(`Hapus Event/Promotion “${item.judul}”? Poster terkait juga akan dihapus.`)) return;
    router.delete(route('admin.event-promo.destroy', item.id), { preserveScroll: true });
};
</script>

<template>
    <Head title="Event & Promotion" />
    <AuthenticatedLayout>
        <template #header><div class="flex flex-wrap items-center justify-between gap-4"><h1 class="text-xl font-semibold leading-tight text-gray-800">Event &amp; Promotion</h1><Link :href="route('admin.event-promo.create')" class="rounded-md bg-primary-blue px-4 py-2 text-sm font-semibold text-white hover:opacity-90">Tambah Event/Promotion</Link></div></template>
