...
const items = [
    ...
    { label: "Wahana & Tempat Makan", route: "wahana" },
...
<template>
    <header
        class="sticky top-0 z-50 px-4 pt-3 transition-all duration-300 lg:px-0 lg:pt-5"
    >
        <nav
            :class="
                scrolled
                    ? 'border-[#dce3ee]/80 bg-white/80 shadow-[0_8px_24px_rgba(0,45,99,.12)] backdrop-blur-md'
        ...
        <div
            v-if="open"
            class="mx-auto max-w-[1120px] rounded-b-2xl border border-t-0 border-[#e1e2eb] bg-white/95 px-5 py-3 shadow-lg backdrop-blur-md md:hidden"
        >
            <Link
                v-for="item in items"
                :key="item.route"
